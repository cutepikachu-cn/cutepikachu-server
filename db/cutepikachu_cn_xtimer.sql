...
(
    ...
    `status`            tinyint unsigned not null default 0 comment '0 NEW 新建，1 ENABLE 激活，2 UNABLE 未激活',
...
(
    ...
    `timer_id`    bigint unsigned  not null comment '定时任务 id',
    `app`         varchar(128)              default null comment '业务方标识',
    `status`      tinyint unsigned not null default 0 comment '0 NOT_RUN 待执行，1 RUNNING 正在执行，2 SUCCESS 执行成功，3 FAIL 执行失败',
