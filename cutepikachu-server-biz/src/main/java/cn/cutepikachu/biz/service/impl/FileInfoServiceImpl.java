...
public class FileInfoServiceImpl extends ServiceImpl<FileInfoMapper, FileInfo> implements IFileInfoService {
    ...
    public FileInfoVO uploadFile(MultipartFile file, FileBizTag bizTag) {
        ...
        try {
            ...
            throw new BusinessException(ResponseCode.INTERNAL_SERVER_ERROR, "读取文件失败");
        }
    ...
    private void checkFileType(MultipartFile file, FileBizTag fileBizTag) {
        ...
        ThrowUtils.throwIf(contentType == null, new BusinessException(ResponseCode.BAD_REQUEST, "文件类型为空"));
        // 校验文件类型
        ...
        ThrowUtils.throwIf(!valid, new BusinessException(ResponseCode.BAD_REQUEST, "文件类型不合法"));
        // 校验文件头魔数
        byte[] bytes;
        try {
            bytes = file.getBytes();
